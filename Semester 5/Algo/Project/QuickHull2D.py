...
# for random number generation and math calculations
import numpy as np
...
def Determinant(point1, point2, point3):
    det = (point1[0]*point2[1]) + (point3[0]*point1[1]) + (point2[0]*point3[1]) - \
    ...
    while (len(points) != 0 and (i < len(points))):
        if (len(list_left) != 0 and len(list_right) != 0):
            ...
            if (not (points[i] in list_right) and not (points[i] in list_left)):
                ...
                # If the point is not in either list, remove it from 'points'
                points.pop(i)
            else:
            ...
            if (not (points[i] in list_right)):
                # If the point is not in 'list_right', remove it from 'points'
                points.pop(i)
            else:
            ...
            if (not points[i] in list_left):
                # If the point is not in 'list_left', remove it from 'points'
                points.pop(i)
            else:
                i = i + 1   # If the point is in 'list_left', move on to the next point
        else:
            # If both 'list_left' and 'list_right' are empty, remove all points in 'points'
            points.pop(i)
...
def QuickHull(left_point, max_point, right_point, points):
    ...
    if (len(points) == 0):
        ...
        if (len(outer_point) > 0):
            ...
            LeftHull = QuickHull(left_point, Far_Left_Point,
                                 max_point, outer_point)
        ...
        if (len(inner_point) > 0):
            Far_Right_Point = Farthest_Point(
                max_point, right_point, inner_point)
            Delete_Point(max_point, right_point, Far_Right_Point, inner_point)
            RightHull = QuickHull(
                max_point, Far_Right_Point, right_point, inner_point)
...
# elapsed time
print("\nElapsed calculation time: ", "{:.8f}".format(
