...
ax.scatter(points[inner_points, 0], points[inner_points, 1], points[inner_points,
           2], c='red', edgecolor='black', linewidths=3, marker='s', s=10)

# Plot the Convex Hull points with the same color but with no border
ax.scatter(hull.points[:, 0], hull.points[:, 1],
           hull.points[:, 2], c='red', marker='X', s=10)
ax.plot_trisurf(hull.points[:, 0], hull.points[:, 1], hull.points[:, 2],
                triangles=hull.simplices, color='grey', alpha=0.35)

hull_points = np.delete(points, inner_points, axis=0)
print("\n------------------------- Convex Hull Points (",
      len(hull_points), "points) -------------------------\n")
print(hull_points)
